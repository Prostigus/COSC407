...
int main(int argc, char* argv[]) {
	   ...
	   long long int number_of_tosses = 50000000;
#pragma omp parallel for num_threads(16)private(a,y,distance_squared) reduction(+:number_in_circle)
	   for(toss = 0; toss < number_of_tosses; toss++) {
		   seed = toss;
		   x = my_rand(&seed);
		   a = my_drand(&x);
		   y = -(my_drand(&x));
		   distance_squared = a * a + y * y;
		   if (distance_squared <= 1)
			   number_in_circle +=1;

	   }
	   double pi_estimate = 4*number_in_circle/((double) number_of_tosses);
	   printf("Pi: %f",pi_estimate);
